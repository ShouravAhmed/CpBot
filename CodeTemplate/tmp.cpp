...
// --- sum of divisor of all number from a to b ---------------------
ll linearSum(ll n) {
    ll x = n;
    ll y = (n + 1);
    if(x&1) y /= 2;
...
ll rangeSum(ll b, ll a) {
    return (linearSum(b) - linearSum(a));
}
 
ll sum_of_divisor_upto_n(ll n) {
     
    ll result = 0;
    ll i = 1;
  
    while(1) {
        result += rangeSum(n / i, n / (i + 1)) * i; 
        if(i == n) break;
        i = n / (n / (i + 1));
    } 
    return result;
...
bool uniun(int a, int b) {
    ...
    for(auto i : components[v]) {
        representative[i] = u;
        components[u].push_back(i);
    }
    components[v] = vi();
...
// Trie :: general peopose template : pointer implementation
...
// Trie :: general peopose template : vector implementation
...
struct my_trie {
    ...
    bool insert(string s) {
        ...
        for(int i = 0; i < s.size(); i++) {
            ...
            if(root[now].next[id] == -1){
                root[now].next[id] = root.size();
                root.push_back(node());
            }

            now = root[now].next[id];
            root[now].path++;

            if(root[now].end) flg = true;
        }
        root[now].end++;
        if(root[now].path > 1) flg = true;
    ...
    bool search(string s) {
        ...
        for(int i = 0; i < s.size(); i++){
            ...
            if(root[now].next[id] == -1) return 0;
            now = root[now].next[id];
        }
        if(root[now].end) return 1;
        return 0;
    }
...
struct my_trie {
    ...
    void insert_name(int k) {
        node *now = root;

        for(int i = 0; i < name[k].size(); i++) {
            int id = name[k][i] -  smallest_ch;
        
            if(now->next[id] == NULL) {
                now->next[id] = new node();
            }

            now->next[id]->dis = now->dis + 1;
            now = now->next[id];
        }
        now->name_idx.push_back(k);
    }
    
    void insert_sudo(int k) {
        node *now = root;

        for(int i = 0; i < sudo[k].size(); i++) {
            int id = sudo[k][i] -  smallest_ch;
        ...
        now->sudo_idx.push_back(k);
    }

    pair<vi, vi> solve(node *now) {
        for(int i = 0; i < 26; i++) {
            if(now->next[i] != NULL) {

                pair<vi, vi> x = solve(now->next[i]);
                if(x.ff.size()) {
                    for(auto j : x.ff) now->name_idx.push_back(j);
                }
                else if(x.ss.size()) {
                    for(auto j : x.ss) now->sudo_idx.push_back(j);
                }
            }
        }

        int mn = min(now->sudo_idx.size(), now->name_idx.size());
        while(mn--) {
            quality += now->dis;
            ans.push_back({now->sudo_idx.back(), now->name_idx.back()});
            now->name_idx.pop_back();
            now->sudo_idx.pop_back();
        }
        return {now->name_idx, now->sudo_idx};
    }
...
int main() {
    ...
    /*
        Names of N boys and N girls are given. You have to make N couple from them.
        for making each couple you will get a point 
        which is the Length of the Largest common prefix for their name

        output the sum of all point for N pair, which is maximum
    */
    ...
    cout << quality << "\n";
    for(auto i : ans) cout << i.ss+1 << " " << i.ff+1 << "\n";
...
class Solution {
public:
    vector<int> inor, preo;
    int pos = 0;
    map<int,int> mp;

    void build(TreeNode *cur, int l, int r) {
        if(l == r) {
            pos++;
            cur->val = inor[l]; 
            return;
    ...
    TreeNode* buildTree(vector<int>& preorder, vector<int>& inorder) {
        ...
        for(int i = 0; i < inor.size(); i++) mp[inor[i]] = i;

        TreeNode *root = new TreeNode();
        build(root, 0, inor.size()-1);
...
vector<edge> mst(int number_of_node, int number_of_edge) {
    vector<edge> ed;
...
// ************ n queen O(1) diagonal check *****************

int n;
int gird[20][20];
int cnt;

bool r1[50], r2[50], r3[50];
...
void solve(int i) {
    ...
    for(int j = 0; j < n; j++) {
        if(gird[i][j] || r1[j] || r2[i + j] || r3[j - i + n - 1]) continue;

        r1[j] = r2[i + j] = r3[j - i + n - 1] = 1;
        solve(i+1);
        r1[j] = r2[i + j] = r3[j - i + n - 1] = 0;
    }
}
...
int max_2d_subarray_sum(vector<vector<int> > ar, int n) {
...
// --------------- fibonacci by fast matrix power ------------------------
...
vector<ull> basepow(1500010, 1);
vector<vector<ull> > fhash(15);
...
inline void HHash() {
    ...
    for(int i = 0; i < vs.size(); i++) {
        ...
        int len = vs[i].size();
        fhash[i].resize(len + 3, 0);
    
        for (int j = 1; j <= len; j++){
            fhash[i][j] = (fhash[i][j - 1] * base + vs[i][j - 1] + 1007);
        }
    }
}

inline ull forward_hash(int idx, int l, int r){
    ull h = fhash[idx][r + 1] - (basepow[r - l + 1] * fhash[idx][l]);
    return h;
}
...
// problem:
// is a substring palindrome with parmanent character update
// segment tree solution

struct node_data {
    ll forward_hash_1, forward_hash_2;
    ll reverse_hash_1, reverse_hash_2;
    node_data() {
        forward_hash_1 = forward_hash_2 = 0;
        reverse_hash_1 = reverse_hash_2 = 0;
    }    
};

string s;
node_data node[400005];

void build(int now, int lo, int hi) {
    if(lo == hi) {
        node[now].forward_hash_1 = (s[lo] + 1007);
        node[now].forward_hash_2 = (s[lo] + 1007);
        node[now].reverse_hash_1 = (s[lo] + 1007);
        node[now].reverse_hash_2 = (s[lo] + 1007);
        return;
    }
    ...
    node[now].forward_hash_1 = ((node[now*2].forward_hash_1 * base_pow_1[hi-mid]) + node[(now*2)+1].forward_hash_1) % mod_1;
    node[now].forward_hash_2 = ((node[now*2].forward_hash_2 * base_pow_2[hi-mid]) + node[(now*2)+1].forward_hash_2) % mod_2;

    node[now].reverse_hash_1 = (node[now*2].reverse_hash_1 + (node[(now*2)+1].reverse_hash_1 * base_pow_1[mid-lo+1])) % mod_1;
    node[now].reverse_hash_2 = (node[now*2].reverse_hash_2 + (node[(now*2)+1].reverse_hash_2 * base_pow_2[mid-lo+1])) % mod_2;  
}

void update(int now, int lo, int hi, int idx, char ch) {
    if(lo == hi) {
        node[now].forward_hash_1 = (ch + 1007);
        node[now].forward_hash_2 = (ch + 1007);
        node[now].reverse_hash_1 = (ch + 1007);
        node[now].reverse_hash_2 = (ch + 1007);
        return;
    }

    int mid = (lo + hi) / 2;

    if(idx <= mid) update(now*2, lo, mid, idx, ch);
    else update((now*2) + 1, mid + 1, hi, idx, ch);
...
node_data query(int now, int lo, int hi, int l, int r) {
    if(lo == l && hi == r) return node[now];
    ...
    if(r <= mid) return query(now * 2, lo, mid, l, r);
        ...
        auto a = query(now * 2, lo, mid, l, mid);
        auto b = query((now * 2) + 1, mid + 1, hi, mid + 1, r);
        auto c = node_data();
        c.forward_hash_1 = ((a.forward_hash_1 * base_pow_1[r-mid]) + b.forward_hash_1) % mod_1;
        c.forward_hash_2 = ((a.forward_hash_2 * base_pow_2[r-mid]) + b.forward_hash_2) % mod_2;
        c.reverse_hash_1 = (a.reverse_hash_1 + (b.reverse_hash_1 * base_pow_1[mid-l+1])) % mod_1;
        c.reverse_hash_2 = (a.reverse_hash_2 + (b.reverse_hash_2 * base_pow_2[mid-l+1])) % mod_2;
...
int main() {
    ...
    while(q--) {
        ...
        if(x == "palindrome?") {
            ...
            int l = II, r = II;
            auto qr = query(1, 1, n, l, r);
            
            if(((qr.forward_hash_1 << 32) + qr.forward_hash_2) == ((qr.reverse_hash_1 << 32) + qr.reverse_hash_2)) {
                printf("Yes\n");
            }
            else printf("No\n");
        }
...
// Two dimensional Hashing template
// search a (x * y) pattern in (n * m) text

struct two_dimensional_string_hashing {
    
    vector<string> text;
    vector<vector<ll> > hash_matrix;

    two_dimensional_string_hashing(vector<string> vs, int n, int m, int x, int y) {
        text = vs;
        hash_matrix = vector<vector<ll> > (n+5, vector<ll>(m+5, 0));

        ll d = 1;
        for(int i = 1; i < y; i++) d = (d * base_1) % mod_1;

        for(int i = 0; i < n; i++) {
            ll P = 0 ;
            for(int j = 0; j < y; j++) P = (P * base_1 + text[i][j]) % mod_1 ;

            for(int j = 0; j <= m - y; j++) {
                
                hash_matrix[i][j] = P ;
                
                P = (((P - text[i][j] * d) % mod_1) * base_1 + text[i][j + y]) % mod_1;
                P = (P + mod_1) % mod_1;
            }
        }

        d = 1 ;
        for(int i = 1; i < x; i++) d = (d * base_2) % mod_2;

        for(int j = 0; j <= m - y; j++) {
            ll P = 0 ;

            for(int i = 0; i < x; i++) P = ( P * base_2 + hash_matrix[i][j] ) % mod_2;

            for(int i = 0; i <= n - x; i++) {
                ll temp = P ;

                P = (((P - hash_matrix[i][j] * d) % mod_2) * base_2 + hash_matrix[i + x][j]) % mod_2;
                P = (P + mod_2) % mod_2;
    ...
    ll hash_value(int x, int y) {
        return hash_matrix[x-1][y-1];
    }
};
// ---------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------
// Zahin vai template - template edited by tanima ***********************************
...
struct manacher {
    ...
    vector<pair<int, int> > get_pal_range() {
        return pal_range;
    }

    void generate_pal_ranges() {
        for(int i = 0, j = 0; i < pal.size(); i += 2, j++) {
                
            if(i+1 < pal.size() && pal[i+1]) {

                int l = j - (pal[i+1]/2) + 1;
                int r = j + (pal[i+1]/2);
                
                pal_range.push_back({l, r});
            } 
...
struct my_trie {
    ...
    void build_fail() {
        ...
        for(int i = 0; i < 26; i++) {
            if(root[0].next[i] != -1) {
                ...
                root[root[0].next[i]].fail = 0;
                q.push(root[0].next[i]);
            }
        }
            ...
            for(int i = 0; i < 26; i++) {
                if(root[u].next[i] != -1) {
                    ...
                    while(root[v].fail == -1) {
                        ...
                        if(root[f].next[i] != -1) {
                            root[v].fail = root[f].next[i];
                        }
                        else{
                            if(f == root[f].fail) root[v].fail = 0;
                            else f = root[f].fail;
                        }
                    }
                }
    ...
    void text_match(string text) {
        int now = 0;
        int len = text.size();
        for(int i = 0; i < len; i++) {
            int id = text[i] - smallest_ch;

            while(now && root[now].next[id] == -1) {
                now = root[now].fail;
            }

            if(root[now].next[id] == -1) continue;
            now = root[now].next[id];
    ...
    void pass_the_frq(int now) {
        int tmp = now;
        int cnt = root[now].lazy;
        root[now].lazy = 0;
        
        while(tmp) {
            root[tmp].frq_of_occur += cnt;
            tmp = root[tmp].fail;
        }

        for(int i = 0; i < 26; i++) {
            if(root[now].next[i] != -1) pass_the_frq(root[now].next[i]);
        }
    }
...
int main() {
    ...
    TEST_CASE{
        ...
        scanf("%s", ch);
        int len = strlen(ch);
        for(int i = 0; i < len; i++) text += ch[i];
        
        for(int i = 0; i < n; i++) {
            scanf("%s", ch);
            len = strlen(ch);
            string s;
            for(int j = 0; j < len; j++) s += ch[j];
            pattern.push_back(s);
        }

        auto tr = my_trie('a');
        for(auto i : pattern) tr.insert(i);

        tr.build_fail();
        tr.text_match(text);

        PRINT_CASEN;
        for(int i = 0; i < n; i++) printf("%d\n", tr.frequency(pattern[i]));
    }
...
******* Lexicographically Minimal cyclic string rotation **********************
...
struct sparse_table{
    ...
    sparse_table(vector<int> &_sparse) {
        sparse.push_back(_sparse);
        len = _sparse.size();
        lg = vector<int>(len+1);
        ...
        while((1 << k) < len) {
            vector<int> row(len);
            for(int i = 0; i < len; i++) {
                row[i] = min(sparse[k][i], (((i + (1 << k)) < len) ? sparse[k][(i + (1 << k))] : 1100000000));
            }
...
// only sorted by the radix sort for the first element only
// lcp array calculated
...
struct suffix_array {
    ...
    int len;
    string s;
    vector<int> sfar, order, lcp;
    sparse_table sp;
 
    suffix_array(string _s) {
        s = _s;
        len = s.size();
        sfar = order = vector<int>(len);
        {
            vector<pair<pair<int,int>, int > > ar(len);
            for(int i = 0; i < len; i++) ar[i] = {{s[i], 0}, i};
            radix_sort(ar);
 
            for(int i = 0; i < len; i++) sfar[i] = ar[i].second;
            order[sfar[0]] = 0;

            for(int i = 1; i < len; i++) {
                if(ar[i-1].first != ar[i].first) order[sfar[i]] = order[sfar[i-1]] + 1;
                else order[sfar[i]] = order[sfar[i-1]];
            }
        }
        {
            int k = 0;
            while((1 << k) < len) {
                ...
                vector<pair<pair<int,int>, int > > ar(len);
                for(int i = 0; i < len; i++) ar[i] = {{order[(sfar[i] - (1<<k) + len) % len], order[sfar[i]]}, (sfar[i] - (1<<k) + len) % len};
                // ar is constructed in a way so that it is already sorted by the second element
                radix_sort(ar);
        ...
        /*
        cout << "suffix array:\n";
        for(int i = 0; i < len; i++) cout << lcp[i] << " :: " << sfar[i] << " :: " << s.substr(sfar[i], len-sfar[i]) << endl;
        */
    }
 
    void radix_sort(vector<pair<pair<int, int>, int> > &ar) {
        // sorting by the first element

        vi cnt(len+512);
        for(auto i : ar) cnt[i.ff.ff]++;
        
        vector<pair<pair<int, int>, int> > new_ar(len);
        vector<int> pos(len+512);

        pos[0] = 0;
        for(int i = 1; i < len+512; i++) {
            pos[i] = pos[i-1] + cnt[i-1];
        }
        
        for(auto i : ar) {
            new_ar[pos[i.ff.ff]] = i;
            pos[i.ff.ff]++;
        }
        ar = new_ar;
    }
    ...
    void calculate_lcp() {
        ...
        for(int i = 0; i < len; i++) {
            ...
            int j = sfar[order[i]-1];
            while(i+equal < len && j+equal < len && s[i+equal] == s[j+equal]) equal++;
    ...
    int longest_common_prefix(int l, int r) {
        
        if(l == r) return (len - r);
        int a = order[l], b = order[r];

        if(a > b) swap(a, b);
        a++;

        return sp.get_min(a, b);        
    }
    ...
    string longest_common_substring(int mid) {
        ...
        for(int i = 0; i < len-1; i++) {
            if((level[i] == 1 && level[i+1] == 2) || (level[i] == 2 && level[i+1] == 1)) {
        ...
        if(sfar[idx] + ret == len) ret--;
        else if(sfar[idx] + ret == mid) ret--;

        return s.substr(sfar[idx], ret);
    }
};
...
// suffix array without complete construction of the 2d array and 
// radix sort for the first and second element
...
struct suffix_array {
    ...
    suffix_array(string s) {
        ...
        sfar = order;
        {
            vector<pair<char, int> > ar(len);
            for(int i = 0; i < len; i++) ar[i] = {s[i], i};
            sort(all(ar));
        ...
        }
        {
            ...
            while((1 << k) < len) {
                ...
                vector<pair<pair<int,int>, int > > ar(len);
                for(int i = 0; i < len; i++) ar[i] = {{order[i],order[(i+(1<<k))%len]}, i};
                radix_sort(ar);
        ...
        cout << "suffix array:\n";
        for(int i = 0; i < len; i++) cout << sfar[i] << " ::: " << s.substr(sfar[i], len-sfar[i]) << endl;
    ...
    void radix_sort(vector<pair<pair<int, int>, int> > &ar) {
        {
            vector<int> cnt(len+265);
            for(auto i : ar) cnt[i.ff.ss]++;
 
            vector<pair<pair<int, int>, int> > new_ar(len);
            vector<int> pos(len+265);
            
            pos[0] = 0;
            for(int i = 1; i < len+265; i++) {
                pos[i] = pos[i-1] + cnt[i-1];
            }
            
            for(auto i : ar) {
                new_ar[pos[i.ff.ss]] = i;
                pos[i.ff.ss]++;
            }
...
// suffix array with complete constrauction of the 2d array or rank of substring 

struct suffix_array {
    vector<vector<int> > sfst;
    vi sfar;
    ...
    suffix_array(string s) {
        ...
        for(int i = 0; i < len; i++) row[i] = (int)s[i];
        sfst.push_back(row);
        
        for(int cur = 0, stp = 1; (1 << cur) < len; cur++, stp++) {
 
            for(int i = 0; i < len; i++) {
                ar[i] = {{sfst[stp-1][i], sfst[stp-1][(i + (1 << cur)) % len]}, i};
            }
            ...
            for(int i = 1; i < len; i++) {
                if(ar[i].ff == ar[i-1].ff) row[ar[i].ss] = row[ar[i-1].ss];
                else row[ar[i].ss] = row[ar[i-1].ss]+1;
            }
        ...
        for(auto i : sfar) {
            cout << i << " " << s.substr(i, len) << endl;       
        }
        cout << "\n";*/
    }   
 
    void radix_sort(vector<pair<pair<int, int>, int> > &ar, int len) {
        {
            vector<int> cnt(len+265);
            for(auto i : ar) cnt[i.ff.ss]++;
    ...
    int lcp(int lo, int hi) {
 
        int ret = 0, up = sfst.size()-1, len = sfst[0].size();
 
        while(up >= 0 && hi < len) {
 
            if(sfst[up][lo] == sfst[up][hi]) {
                ret += (1 << up);
                lo += (1 << up);
                hi += (1 << up);
            }
...
// minimum swap needed to reach a string "S" from a string "T"

vi used;
vi node;

void update(int now, int lo, int hi, int idx) {
    if(lo == hi) {
        node[now]++;
        return;
    }
    ...
    if(idx <= mid) update(now * 2, lo, mid, idx);
    else update((now * 2) + 1, mid + 1, hi, idx);

    node[now] = node[now * 2] + node[(now * 2) + 1];
}

int query(int now, int lo, int hi, int l, int r) {
    if(lo == l && r == hi) return node[now];

    int mid = (lo + hi) / 2;

    if(l > mid) return query((now * 2) + 1, mid + 1, hi, l, r);
    else if(r <= mid) return query(now * 2, lo, mid, l, r);
    else {
        return (query(now * 2, lo, mid, l, mid) 
            + query((now * 2) + 1, mid + 1, hi, mid + 1, r));
    }
}

ll swap_needed(const string &s, const string &t) {
    ...
    for(int i = 0; i < n; i++) {
        ...
        int taken = 0;
        if(idx - 1 >= 0) taken = query(1, 0, n - 1, 0, idx - 1);
        
        int cur_idx = idx - taken;
